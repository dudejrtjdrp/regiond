...
test('§14-7 열린 갈래 안의 잠긴 건물은 까닭과 함께 실린다', () => {
  ...
  /* ★ §21-C3 — 여기는 예전에 `/티어 4/` 를 봤다. 목장이 **어느 장에도 안 적혀 있어서** 「마지막 장 뒤
     티어 4에서 해금」이라는, 사실이지만 아무 도움이 안 되는 안내가 나가던 시절의 기대다.
     지금 목장은 7장이 연다 — 안내도 그 자리를 가리켜야 한다. 재는 것은 그대로다: 잠긴 것도 목록에
     남고 까닭이 글로 나온다(§12-3). 달라진 것은 그 까닭이 **사람이 갈 수 있는 곳**을 가리킨다는 것뿐이다. */
  assert.match(ranch.lockReason, /7장/, `목장 해금 조건: ${ranch.lockReason}`);
});
