...
test('효과 합산 — 동종은 더하되 그 건물 최고 티어 값이 상한이다 (밸런스 보호)', () => {
  ...
  assert.equal(capValue, 0.5, 'T3 곡창의 +50% 가 상한');
  put(w, n, 'granary', 1, 3);
  assert.ok(Math.abs(effectValue(n, 'output.grain', data) - 0.15) < 1e-9);
  put(w, n, 'granary', 1, 6);
  assert.ok(Math.abs(effectValue(n, 'output.grain', data) - 0.3) < 1e-9, '두 채면 합산된다');
  put(w, n, 'granary', 3, 9);
