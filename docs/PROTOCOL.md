# 클라-서버 프로토콜 v3.1 (양측 구현 계약 — 이 파일이 최종 정본)

> 임의 변경 금지. 바꿔야 하면 **이 파일을 먼저 고치고** 서버·클라를 따라 고친다.
> 서버 판번호는 `server/index.js` 의 `PROTOCOL = '3.1'`, 클라는 `public/js/state.js` 의 `GM.PROTOCOL` 과 **반드시 같아야** 한다.
> 설계 근거는 `docs/GDD3.md`(§1~§10 엔드리스 정착지 성장 · **§11 진행 감독** · **§12 플레이테스트 1차**), 공간 계층은 `docs/WORLD.md`.
...
`features` 키(클라 동작 분기): `gather` `swing` `reclaim` `placeBuilding` `chronicle` · `residentArrival` `commandVillagers` `reclaimField` · `fences` `waves` `prophecyHint` · `emotionDay` `roles` `trade` `departments` `advisor` · `orders` `council` `artifacts` · `highTierUpgrade` `diplomacy` · `prestige`

---

## 7. 세이브 정책

`world.schema = 4`. v1(8×8 타일)·v2(시즌 오픈월드)·**v3(128×128 · 티어 해금)** 스냅샷은 **읽지 않는다** —
v3.1 에서 월드가 256×256 이 되고 해금의 정본이 장(chapter)으로 옮겨졌기 때문이다.
...
| 웨이브5 생존율(권장 방어) | 70.0% | 60~80% | PASS |
| 식량 파산율 | 0.0% | <5% | PASS |
| 성녀 유무 웨이브8 격차 | 25.0%p | ≥20%p | PASS |
